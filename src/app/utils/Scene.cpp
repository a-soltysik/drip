...
{
auto Scene::createDefaultScene(gfx::Context& context, const sim::SimulationConfig& simulationParameters)
    -> std::unique_ptr<Scene>
{
    ...
    domainRenderable->transform.scale = simulationParameters.domain.bounds.max - simulationParameters.domain.bounds.min;
    domainRenderable->transform.translation =
        (simulationParameters.domain.bounds.min + simulationParameters.domain.bounds.max) / 2.F;
    ...
    const auto fluidScale = simulationParameters.fluid.bounds.max - simulationParameters.fluid.bounds.min;
    const auto particleCount = glm::uvec3 {glm::floor(fluidScale / simulationParameters.fluid.properties.spacing)};
    scene.addRenderable(
    ...
    return std::unique_ptr<Scene> {new Scene {std::move(scene)}};
}

Scene::Scene(gfx::Scene scene)
